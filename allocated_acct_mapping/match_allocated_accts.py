...
class match_leads_to_accounts(object):
    ...
    def __init__(self, lead_file, account_file, out_file, marketo_upload=False, picklepath = "C:/Users/wkuffel/Desktop/update unranked/Target accounts/parent_account_dict.p"):
        ...
        self.write_fields = ['Id', 'First Name','Last Name', 'Full Name', 'Email Address', 'InsideView Account ID', 'Phone Number', 'SFDC Type', 'Company Name', 'Job Title','Annual Company Revenue Range', 'Company Industries', 'Annual Company Revenue Range (A)', 'Company Employee Range',   'Lead Status', 'Lead Source', 'Updated At', 'Lead Score', 'Decision Maker', 'Job Level', 'Sales Title', 'Marketing Title', 'BI Title', 'OEM Title', 'Analytics Title', 'Operations Title', 'Clean Company Name1', 'HQ Country','Country','Region', 'State', 'HQ State',
                             'Matched',
        ...
        for marketing_row in self.second_pass_new_ds:
            ...
            if marketing_row["Matched"] == True:
                for account_row in self.target_accounts_ds:
                    if account_row["ID"] == marketing_row["Matched Account ID"]:
            ...
            if marketing_row['Matched'] == False:
                for account_row in self.target_accounts_ds:
                    if marketing_row["Clean Company Name1"] == account_row["Clean Company Name"]:
                        ...
                        marketing_row["Matched Account ID"] = account_row['ID']
                        marketing_row["Matched is_child"] = account_row['is_child']
                        marketing_row["Matched is_parent"] = account_row['is_parent']
                        marketing_row["Matched Parent Account ID"] = account_row["Parent ID"]
    ...
    def write_to_csv(self):
        ...
        if self.marketo_upload == True:
            print "here"
            self.write_fields = ["Matched", "Email Address", "Sales Title", "Marketing Title", "OEM Title", "BI Title", "Analytics Title","Operations Title", "Job Level", "Decision Maker", "Lead Rank"]
            self.additional_fields = [ 'Account ID']
            final_ds = []
